import { Box, Edit, Loader2, Moon, Plus, Sun, Trash2 } from 'lucide-react';
...
const Notes = () => {
  ...
  const renderContent = () => {
    ...
    if (error) {
      return (
        <div className='mb-6 p-4 bg-green-500/10 text-error rounded-lg border border-tertiary flex items-center gap-3'>
          <svg
    ...
    if (notes?.length === 0) {
      return (
        <div className='flex flex-col items-center justify-center bg-green-800 p-8 rounded-2xl shadow-sm border border-tertiary text-center'>
    ...
    return (
      <div className='space-y-4'>
        {notes?.map(note => (
          <div
            key={note._id}
            className='group bg-green-400 rounded-xl p-4 shadow-sm border border-tertiary flex justify-between items-center transition-all hover:shadow-md hover:bg-red-600/80 group'
          >
            ...
            >
              <button
                ...
                className='p-2 text-tertiary hover:bg-yellow-500 rounded-lg transition-colors'
                title='Edit note'
              >
  ...
  return (
    <main className='min-h-screen p-6 md:p-10 bg-green-950'>
